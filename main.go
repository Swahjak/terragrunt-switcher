...
* 3- Unzip the file to /usr/local/terragrunt
...
func main() {
	...
	custBinPath := getopt.StringLong("bin", 'b', lib.ConvertExecutableExt(defaultBin), "Custom binary path. Ex: tgswitch -b "+lib.ConvertExecutableExt("/Users/username/bin/terraform"))
	...
	chDirPath := getopt.StringLong("chdir", 'c', dir, "Switch to a different working directory before executing the given command. Ex: tgswitch --chdir terraform_project will run tgswitch in the terraform_project directory")
	...
	case fileExists(TOMLConfigFile) || fileExists(HomeTOMLConfigFile):
		...
		case fileExists(RCFile) && len(args) == 0:
			...
			installVersion(tgversion, &binPath, mirrorURL)
		...
		case checkTGModuleFileExist(*chDirPath) && len(args) == 0:
			installTGProvidedModule(*chDirPath, &binPath, mirrorURL)
		...
		case checkTgEnvExist() && len(args) == 0 && version == "":
			...
			installVersion(tgversion, custBinPath, mirrorURL)
	...
	case checkTGModuleFileExist(*chDirPath) && len(args) == 0:
		installTGProvidedModule(*chDirPath, custBinPath, mirrorURL)
...
func installLatestVersion(custBinPath, mirrorURL *string) {
	tgversion, _ := lib.GetTGLatest(*mirrorURL)
...
func installLatestImplicitVersion(requestedVersion string, custBinPath, mirrorURL *string, preRelease bool) {
	...
	tgversion, err := lib.GetTGLatestImplicit(*mirrorURL, preRelease, requestedVersion)
...
func showLatestImplicitVersion(requestedVersion string, custBinPath, mirrorURL *string, preRelease bool) {
	if lib.ValidMinorVersionFormat(requestedVersion) {
		tgversion, _ := lib.GetTGLatestImplicit(*mirrorURL, preRelease, requestedVersion)
		if len(tgversion) > 0 {
			fmt.Printf("%s\n", tgversion)
		} else {
...
func installVersion(arg string, custBinPath *string, mirrorURL *string) {
	if lib.ValidVersionFormat(arg) {
		...
		listAll := true                                     //set list all true - all versions including beta and rc will be displayed
		tglist, _ := lib.GetTGList(*mirrorURL, listAll)     //get list of versions
...
func checkTGModuleFileExist(dir string) bool {
	...
	module, _ := tgconfig.LoadModule(dir)
...
func installTGProvidedModule(dir string, custBinPath, mirrorURL *string) {
	...
	installFromConstraint(&tgconstraint, custBinPath, mirrorURL)
}
